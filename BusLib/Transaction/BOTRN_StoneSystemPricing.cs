...
{
    public class BOTRN_StoneSystemPricing
    {
        AxonDataLib.BOSQLHelper Ope = new AxonDataLib.BOSQLHelper();
        AxonDataLib.BOConversion Val = new AxonDataLib.BOConversion();    

        public DataTable GetStoneSystemPricingData(LiveStockProperty pClsProperty, string pStrRapDate)
        {
            Ope.ClearParams();
            ...
            Ope.FillDTab(Config.ConnectionString, Config.ProviderName, DTab, "Pri_StoneSystemPricingGetData", CommandType.StoredProcedure);
            return DTab;
        }
        ...
        public MemoEntryProperty SaveStoneSystemPriceUpdateDetail(string StrPriceParameteDetXML, bool ISSALEPriceUpdate, bool ISMFGPriceUpdate, bool ISCOSTPriceUpdate, bool ISAVGPriceUpdate) //#P : 10-03-2022
        {
            MemoEntryProperty pClsProperty = new MemoEntryProperty();
            try
            {
                ...
                Ope.AddParams("ISSALEPRICEUPDATE", ISSALEPriceUpdate, DbType.Boolean, ParameterDirection.Input);
                Ope.AddParams("ISMFGPRICEUPDATE", ISMFGPriceUpdate, DbType.Boolean, ParameterDirection.Input);
                Ope.AddParams("ISCOSTPRICEUPDATE", ISCOSTPriceUpdate, DbType.Boolean, ParameterDirection.Input);
                Ope.AddParams("ISAVGPRICEUPDATE", ISAVGPriceUpdate, DbType.Boolean, ParameterDirection.Input);
                Ope.AddParams("ENTRYBY", Config.gEmployeeProperty.LEDGER_ID, DbType.Guid, ParameterDirection.Input);
                Ope.AddParams("ENTRYIP", Config.ComputerIP, DbType.String, ParameterDirection.Input);
                Ope.AddParams("RETURNVALUE", "", DbType.String, ParameterDirection.Output);
                Ope.AddParams("RETURNMESSAGETYPE", "", DbType.String, ParameterDirection.Output);
                ...
                ArrayList AL = Ope.ExeNonQueryWithOutParameter(Config.ConnectionString, Config.ProviderName, "Pri_StoneSystemPricingUpdate", CommandType.StoredProcedure);
                if (AL.Count != 0)
                {
                    pClsProperty.ReturnValue = Val.ToString(AL[0]);
                    pClsProperty.ReturnMessageType = Val.ToString(AL[1]);
