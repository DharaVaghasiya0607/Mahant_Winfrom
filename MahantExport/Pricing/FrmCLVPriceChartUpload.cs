...
{
    public partial class FrmCLVPriceChartUpload : DevControlLib.cDevXtraForm
    {
        ...
        private void BtnUpdateDiscFileWise_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (Val.ToString(txtParameter.Text) == "ORIGINAL_MIXRATE")
                    {
                        //---------- For MixRate----
                        if (IntRow % 14 == 0)
                        {
                    ...
                    for (int IntCol = 0; IntCol < DTabFile.Columns.Count; )
                    {
                        ...
                        if (Val.ToString(DTabFile.Rows[IntShapeRowIndex][IntShapeColIndex]) == "" && Val.Val(Val.ToString(DTabFile.Rows[IntSizeRowIndex][IntSizeColIndex]).Split('-')[0]) == 0) //Koi Box ma Shape and Size Blank hase to Box j Consider na thay
                        {
                            IntParaMeterColIndex = IntCol;
                            IntShapeColIndex = IntCol + 1;
                            IntColorColIndex = IntCol;
                        ...
                        DRNew["FCARAT"] = Val.Val(Val.ToString(DTabFile.Rows[IntSizeRowIndex][IntSizeColIndex]).Split('-')[0]);
                        DRNew["TCARAT"] = Val.Val(Val.ToString(DTabFile.Rows[IntSizeRowIndex][IntSizeColIndex]).Split('-')[1]);
        ...
        private void lblSampleFileDownload_Click(object sender, EventArgs e)
        {
            ...
            System.Diagnostics.Process.Start(Application.StartupPath + "\\Format\\CLVPricePerCaratFormat.xlsx", "CMD");
        }
        ...
        private void BtnExportAllParameter_Click(object sender, EventArgs e)
        {
            //try
            //{
            //    if (Val.ToString(txtRapDate.Text) == "")
            ...
            //    ExportWithParameter(DTabDiscountData);
            ...
            try
            {
                ...
                DTabDiscountData = ObjTrn.GetParameterDiscountData("EXPORTEXCEL", Val.ToString(txtParameter.Tag), Val.SqlDate(txtRapDate.Text), Val.ToString(txtShape.Text), DouFromSize, DouToSize, Val.ToString(CmbPricingType.Text));
        ...
        private void ExportWithParameter(DataTable DTabDiscountData)
        {
            try
            {
                if (DTabDiscountData.Rows.Count == 0)
                ...
                using (ExcelPackage xlPackage = new ExcelPackage(workBook))
                {
                    DataTable DTabParameterSheet = DTabDiscountData.DefaultView.ToTable(true, "PARAMETER_ID");
                    foreach (DataRow DR in DTabParameterSheet.Rows)
                    {
                        ExcelWorksheet worksheet = xlPackage.Workbook.Worksheets.Add(Val.ToString(DR["PARAMETER_ID"]));

                        StartRow = 1;
                        EndRow = StartRow;
                        StartColumn = 1;

                        DataTable DTabDiscountDistinct = DTabDiscountData.DefaultView.ToTable(true, "SIZE", "PARAMETER_ID", "PARAMETER_VALUE", "SHAPE_CODE");
                        DTabDiscountDistinct = DTabDiscountDistinct.Select("PARAMETER_ID = '" + Val.ToString(DR["PARAMETER_ID"]) + "'").CopyToDataTable();

                        DTabDiscountDistinct.DefaultView.Sort = "PARAMETER_ID,PARAMETER_VALUE";
                        DTabDiscountDistinct = DTabDiscountDistinct.DefaultView.ToTable();

                        DataTable DTabDiscountDistinctParamaterValue = DTabDiscountDistinct.DefaultView.ToTable(true, "PARAMETER_ID", "PARAMETER_VALUE");

                        int intCnt = 0;
                        int StartColumnPara = 1;
                        foreach (DataRow DRParam in DTabDiscountDistinctParamaterValue.Rows)
                        {
                            DataTable DTabParameterDis = DTabDiscountDistinct.Select("PARAMETER_ID = '" + Val.ToString(DRParam["PARAMETER_ID"]) + "' And PARAMETER_VALUE = '" + Val.ToString(DRParam["PARAMETER_VALUE"]) + "'").CopyToDataTable();

                            StartRow = 1;
                            intCnt = 0;

                            foreach (DataRow DRowDisc in DTabParameterDis.Rows)
                            {
                                if (intCnt == 0)
                                {
                                    intCnt = 1;
                                    //StartColumn = 1;
                                    //StartHeaderColumn = 1;
                                    StartColumn = StartColumnPara;
                                    StartHeaderColumn = StartColumnPara;
                                ...
                                {
                                    intCnt = 0;
                                    StartColumn = StartColumnPara + 18;
                                    StartHeaderColumn = StartColumnPara + 18;
                                    StartRow = EndRow;
                                }
                                //if (intCnt == 0)
                                //{
                                ...
                                //        StartHeaderColumn = StartHeaderColumn + 18;
                                //        StartRow = EndRow;
                                //        intCnt++;
                                //    }
                                //}
                                ...
                                string StrSize = Val.ToString(DRowDisc["SIZE"]);
                                string StrParameterValue = Val.ToString(DRowDisc["PARAMETER_VALUE"]);
                                string StrShape = Val.ToString(DRowDisc["SHAPE_CODE"]);

                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn].Value = StrParameterValue;
                                worksheet.Cells[StartRow, StartColumn + 1, StartRow, StartColumn + 16].Value = StrShape;
                                // worksheet.Cells[StartRow, StartColumn + 1, StartRow, StartColumn + 16].Merge = true;

                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn].Style.Fill.PatternColor.SetColor(Color.Yellow);
                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn].Style.Fill.BackgroundColor.SetColor(Color.Yellow);

                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.VerticalAlignment = OfficeOpenXml.Style.ExcelVerticalAlignment.Center;

                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.Border.Left.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;
                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.Border.Right.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;
                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.Border.Top.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;
                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.Font.Name = FontName;
                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.Font.Size = FontSize;
                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.Border.Bottom.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;

                                worksheet.Cells[StartRow, StartColumn, StartRow, StartColumn + 16].Style.Font.Bold = true;

                                worksheet.Cells[StartRow, StartColumn + 1, StartRow + 1, StartColumn + 16].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                                worksheet.Cells[StartRow, StartColumn + 1, StartRow + 1, StartColumn + 16].Style.Fill.PatternColor.SetColor(Color.FromArgb(156, 195, 230));
                                worksheet.Cells[StartRow, StartColumn + 1, StartRow + 1, StartColumn + 16].Style.Fill.BackgroundColor.SetColor(Color.FromArgb(156, 195, 230));
                                worksheet.Cells[StartRow, StartColumn + 1, StartRow + 1, StartColumn + 16].Style.Font.Color.SetColor(FontColor);

                                StartRow = StartRow + 1;
                                StartColumn = StartColumn + 1;

                                //DataRow[] UDRow = DTabDiscountData.Select("SIZE='" + StrSize + "' AND PARAMETER_ID = '" + Val.ToString(DR["PARAMETER_ID"]) + "' AND SHAPE_CODE = '" + StrShape + "'");
                                DataRow[] UDRow = DTabDiscountData.Select("SIZE='" + StrSize + "' AND PARAMETER_ID = '" + Val.ToString(DR["PARAMETER_ID"]) + "' AND SHAPE_CODE = '" + StrShape + "' And PARAMETER_VALUE = '" + StrParameterValue + "'");
                                if (UDRow.Length == 0)
                                {
                                    continue;
                                }
                                ...
                                worksheet.Cells[StartRow, StartColumn + 10, StartRow, StartColumn + 10].Value = "I2";
                                worksheet.Cells[StartRow, StartColumn + 11, StartRow, StartColumn + 11].Value = "I3";
                                worksheet.Cells[StartRow, StartColumn + 12, StartRow, StartColumn + 12].Value = "I4";
                                worksheet.Cells[StartRow, StartColumn + 13, StartRow, StartColumn + 13].Value = "I5";
                                worksheet.Cells[StartRow, StartColumn + 14, StartRow, StartColumn + 14].Value = "I6";
                                worksheet.Cells[StartRow, StartColumn + 15, StartRow, StartColumn + 15].Value = "I7";
                                worksheet.Cells[StartRow, StartColumn - 1, StartRow, StartColumn + 15].Style.Font.Bold = true;
                                ...
                                foreach (DataRow DRowDetail in UDRow)
                                {
                                    ...
                                    worksheet.Cells[StartRow, StartColumn + 10, StartRow, StartColumn + 10].Value = Val.Val(DRowDetail["Q11"]);
                                    worksheet.Cells[StartRow, StartColumn + 11, StartRow, StartColumn + 11].Value = Val.Val(DRowDetail["Q12"]);
                                    worksheet.Cells[StartRow, StartColumn + 12, StartRow, StartColumn + 12].Value = Val.Val(DRowDetail["Q13"]);
                                    worksheet.Cells[StartRow, StartColumn + 13, StartRow, StartColumn + 13].Value = Val.Val(DRowDetail["Q14"]);
                                    worksheet.Cells[StartRow, StartColumn + 14, StartRow, StartColumn + 14].Value = Val.Val(DRowDetail["Q15"]);
                                    worksheet.Cells[StartRow, StartColumn + 15, StartRow, StartColumn + 15].Value = Val.Val(DRowDetail["Q16"]);

                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Border.Bottom.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;
                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Border.Left.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;
                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Border.Right.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;
                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Border.Top.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;
                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.VerticalAlignment = OfficeOpenXml.Style.ExcelVerticalAlignment.Center;
                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Font.Name = FontName;
                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Font.Size = FontSize;
                                    //worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                                    //worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Fill.PatternColor.SetColor(Color.FromArgb(225, 150, 150));
                                    //worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Fill.BackgroundColor.SetColor(Color.FromArgb(225, 150, 150));
                                    worksheet.Cells[StartRow - 1, StartColumn - 1, StartRow, StartColumn + 15].Style.Font.Color.SetColor(FontColor);
                                    worksheet.Cells[StartRow - 1, StartHeaderColumn, StartRow, StartHeaderColumn].Style.Font.Bold = true;
                                    worksheet.Cells[StartRow - 1, StartHeaderColumn, StartRow, StartHeaderColumn].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                                    worksheet.Cells[StartRow - 1, StartHeaderColumn, StartRow, StartHeaderColumn].Style.Fill.PatternColor.SetColor(Color.FromArgb(156, 195, 230));
                                    worksheet.Cells[StartRow - 1, StartHeaderColumn, StartRow, StartHeaderColumn].Style.Fill.BackgroundColor.SetColor(Color.FromArgb(156, 195, 230));
                            ...
                            StartColumnPara = StartColumnPara + 36;
                        }
                    }
                    xlPackage.Save();
        ...
        private void ExportWithParameter_OLD(DataTable DTabDiscountData)
        {
            try
            {
                if (DTabDiscountData.Rows.Count == 0)
                ...
                using (ExcelPackage xlPackage = new ExcelPackage(workBook))
                {
                    ...
                    foreach (DataRow DR in DTabParameterSheet.Rows)
                    {
                        ...
                        DataTable DTabDiscountDistinctParamaterValue = DTabDiscountData.DefaultView.ToTable(true, "PARAMETER_ID", "PARAMETER_VALUE");
