...
{
    public partial class FrmInvoiceEntry : DevControlLib.cDevXtraForm
    {
        ...
        public void ShowForm(FORMTYPE pFormType)
        {
            ...
            if (MainGrdSummury.RepositoryItems.Count == 6)
            {
                ObjGridSelection = new BODevGridSelection();
                ObjGridSelection.View = GrdSummury;
            ...
            GrdSummury.Columns["COLSELECTCHECKBOX"].Fixed = DevExpress.XtraGrid.Columns.FixedStyle.Right;
            ...
            try
            {
                ...
                foreach (DataRow DRow in pDtInvoice.Rows)
                {
                    if (DRow != null)
                    {
                        ...
                        DRNew["RATEHKD"] = Val.ToDouble(DRow["MEMOPRICEPERCARAT"]) * Val.ToDouble(txtHKDRate.Text); //Added by Daksha on 12/07/2023
                        //DRNew["RATEHKD"] = DRow["FMEMOPRICEPERCARAT"];
                        DRNew["RATEUSD"] = DRow["MEMOPRICEPERCARAT"];
                        DRNew["AMOUNTHKD"] = Val.ToDouble(DRow["MEMOAMOUNT"]) * Val.ToDouble(txtHKDRate.Text); //Added by Daksha on 12/07/2023
        ...
        public void ShowForm(string HKINVOICE_ID)  //Call When Click Branch Receive HK INVOICE NO
        {
            try
            {
                Val.FormGeneralSetting(this);
                ...
                INVOICE_ID = HKINVOICE_ID;
                //mFormType = pFormType;
                FillControlName();
                this.Show();

                DataTable DTInvoiceData = ObjInvoice.FillSummaryBYID(INVOICE_ID);

                txtInvoiceNo.Text = Val.ToString(DTInvoiceData.Rows[0]["INVOICENO"]);
                txtInvoiceNoStr.Text = Val.ToString(DTInvoiceData.Rows[0]["INVOICENOSTR"]);
                txtInvoiceNo.Tag = Val.ToString(DTInvoiceData.Rows[0]["INVOICE_ID"]);
                DTPInvoiceDate.Text = Val.ToString(DTInvoiceData.Rows[0]["INVOICEDATE"]);
                txtParty.Tag = Val.ToString(DTInvoiceData.Rows[0]["PARTY_ID"]);
                txtParty.Text = Val.ToString(DTInvoiceData.Rows[0]["PARTYNAME"]);
                txtSalePerson.Tag = Val.ToString(DTInvoiceData.Rows[0]["SELLER_ID"]);
                txtSalePerson.Text = Val.ToString(DTInvoiceData.Rows[0]["SELLERNAME"]);
                txtBRNo.Text = Val.ToString(DTInvoiceData.Rows[0]["BRNO"]);
                txtAuthorisePerson.Tag = Val.ToString(DTInvoiceData.Rows[0]["AUTHORISEPERSON_ID"]);
                txtAuthorisePerson.Text = Val.ToString(DTInvoiceData.Rows[0]["AUTHORISEPERSONNAME"]);
                txtTerms.Tag = Val.ToString(DTInvoiceData.Rows[0]["TERMS_ID"]);
                txtTerms.Text = Val.ToString(DTInvoiceData.Rows[0]["TERMSNAME"]);
                txtPhNo.Text = Val.ToString(DTInvoiceData.Rows[0]["PHNO"]);
                txtRemark.Text = Val.ToString(DTInvoiceData.Rows[0]["REMARK"]);
                if (Val.ToString(DTInvoiceData.Rows[0]["ISPickup"]) == "YES")
                    ChkOrderConfirmPickup.Checked = true;
                else
                    ChkOrderConfirmPickup.Checked = false;

                if (Val.ToString(DTInvoiceData.Rows[0]["CURRENCYTYPE"]) == Val.ToString(RbUSD.Tag.ToString()))
                    RbUSD.Checked = true;
                else
                    RbHKD.Checked = true;

                if (Val.ToString(DTInvoiceData.Rows[0]["BANKTYPE"]) == Val.ToString(rbDBS.Tag.ToString()))
                    rbDBS.Checked = true;
                else
                    rbHSBC.Checked = true;

                ChkPayment.Checked = Val.ToBoolean(DTInvoiceData.Rows[0]["IS_Payment"]); //Added by Daksha on 12/07/2023
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //Added by Daksha on 12/07/2023
                if (RbHKD.Checked)
                {
                    if (Val.ToDouble(txtHKDRate.Text)==0)
                    {
                        Global.Message("HKD Rate Is Requried");
                        txtHKDRate.Focus();
                        return;
                    }
                }
                ...
                Property.IS_Payment = ChkPayment.Checked;//Gunjan:23/05/2023
                Property.ExcRate = Val.ToDouble(txtHKDRate.Text); //Added by Dakssha on 12/07/2023

                Property = ObjInvoice.Save(Property);
                
                if (Property.ReturnMessageType == "SUCCESS") //Added Condition by Daksha on 16/03/2023 'Bcoz When fail it assign -1 value
                {
        ...
        private void BtnClear_Click(object sender, EventArgs e)
        {
            ...
            lblMode.Text = "Add Mode";
            ChkOrderConfirmPickup.Checked = false;
            ChkPayment.Checked = false;//gunjan:23/05/2023
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Cursor = Cursors.WaitCursor;
                DataTable DTabSummury = ObjInvoice.FillSummary(
        ...
        private void GrdDet_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                txtHKDRate_TextChanged(null, null); //Added by Daksha on 12/07/2023
                CalculationNewInvoiceEntry(); //Add shiv 09-11-22
        ...
        private void GrdSummury_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            ...
            if (e.Clicks == 2)
            {
                ...
                ChkPayment.Checked = Val.ToBoolean(DRow["IS_PAYMENT"]);//Gunjan:23/05/2023

                txtHKDRate.Text = Val.ToString(DRow["EXCRATE"]); //Added by Daksha on 12/07/2023
        ...
        {
            if (Val.ToInt(txtInvoiceNo.Text) == 0)
            {
            ...
            string Ope = "";
            if (RbtPrintDetail.Checked == true)
            {
                Ope = "DETAIL";
            }
            else
            ...
            DataTable DTabPrint = ObjInvoice.Print(Val.ToString(txtInvoiceNo.Tag), Ope);
            ...
            {
                StrRptName = Ope == "SUMMARY" ? "MemoInvoiceHK_Summary" : "MemoInvoiceHK";

                FrmReportViewer.ShowMemoInvoiceHKPrint(StrRptName, DTabPrint, txtParty.Text, txtInvoiceNo.Text);
            ...
            {
                StrRptName = Ope == "SUMMARY" ? "SaleInvoiceHK_Summary" : "SaleInvoiceHK";
        ...
        private DataTable GetTableOfSelectedRows(GridView view, Boolean IsSelect, BODevGridSelection pSelectGrid)//Gunjan:23/05/2023
        {
            if (view.RowCount <= 0)
            {
                return null;
        ...
        private void paymentToolStripMenuItem_Click(object sender, EventArgs e)//Gunjan:23/05/2023
        {
            try
            {
                DataTable dtCancel = GetTableOfSelectedRows(GrdSummury, true, ObjGridSelection);
                string strInvoiceId = "";
                ...
                {
                    var list = dtCancel.AsEnumerable().Select(r => r["INVOICE_ID"].ToString());
                    strInvoiceId = string.Join(",", list);
                }
                else
                {
                    strInvoiceId = Val.ToString(GrdSummury.GetFocusedRowCellValue("INVOICE_ID"));
                }

                TRN_InvoiceProperty Property = new TRN_InvoiceProperty();
                Property = ObjInvoice.UpdateIS_Payment(Property, strInvoiceId);
        ...
        private void GrdSummury_MouseUp(object sender, MouseEventArgs e)
        {

            if (GrdSummury.FocusedColumn == GrdSummury.Columns["IS_PAYMENT"])
            {
                paymentToolStripMenuItem.Visible = true;
                InvoicePaymentDoneToolStripMenuItem.Visible = false;
            }
            else
            {
                paymentToolStripMenuItem.Visible = false;
                InvoicePaymentDoneToolStripMenuItem.Visible = true;
            }
        }
        ...
        private void txtHKDRate_TextChanged(object sender, EventArgs e)        
        {
            try
            {
                GrdDet.CellValueChanged -= GrdDet_CellValueChanged;
                for (int i = 0; i < GrdDet.RowCount; i++)
                {
                    double PricePerCarat = Val.ToDouble(GrdDet.GetRowCellValue(i, "RATEUSD"));
                    double Amount = Val.ToDouble(GrdDet.GetRowCellValue(i, "AMOUNTUSD"));
                    GrdDet.SetRowCellValue(i, "RATEHKD", PricePerCarat * Val.ToDouble(txtHKDRate.Text));
                    GrdDet.SetRowCellValue(i, "AMOUNTHKD", Amount * Val.ToDouble(txtHKDRate.Text));
                }
                GrdDet.CellValueChanged += GrdDet_CellValueChanged;
            }
            catch (Exception ex)
        ...
        private void txtHKDRate_Validated(object sender, EventArgs e)
        {
            txtHKDRate_TextChanged(null, null);
        }
        //End as Daksha
    }
}
