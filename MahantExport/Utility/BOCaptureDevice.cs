...
namespace MahantExport.Utility
{
    ...
    public class BOCaptureDevice
    {
        ...
        private BOCaptureDevice(ushort deviceNumber, string name, string description)
        {
        ...
        public static List<BOCaptureDevice> GetDevices()
        {
            List<BOCaptureDevice> devices = new List<BOCaptureDevice>();
            ...
            for (ushort i = 0; i < MAX_DEVICES; ++i)
            {
                ...
                {
                    devices.Add(new BOCaptureDevice(i, name.ToString(), description.ToString()));
                }
            }
